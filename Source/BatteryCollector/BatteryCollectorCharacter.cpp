...
ABatteryCollectorCharacter::ABatteryCollectorCharacter()
{
	...
	collectionSphereComponent = CreateDefaultSubobject<USphereComponent>(TEXT("CollectionSphere"));
	collectionSphereComponent->AttachToComponent(RootComponent, FAttachmentTransformRules::KeepWorldTransform);
...
void ABatteryCollectorCharacter::CollectPickups()
{
	...
	collectionSphereComponent->GetOverlappingActors(batteryPickups, ABatteryPawn::StaticClass());
	for (int i = 0; i < batteryPickups.Num(); i++)
	{
		ABatteryPawn* batteryPickup = Cast<ABatteryPawn>(batteryPickups[i]);
		batteryPickup->WasCollected();
	}

}


void ABatteryCollectorCharacter::updatePowerBy(float deltaPower)
{
}

float ABatteryCollectorCharacter::getPower()
{
	return power;
}
